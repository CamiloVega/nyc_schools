...
class SchoolListAdapter @Inject constructor() : PagedListAdapter<SchoolDisplayData, SchoolListAdapter.SchoolItemViewHolder>(DiffCallback()) {
  ...
  class SchoolItemViewHolder(view: View) : RecyclerView.ViewHolder(view){
    fun  bind(school: SchoolDisplayData) {
      ...
      itemView.setOnClickListener { school.onClick() }
    }
  }
...
data class SchoolDisplayData(val schoolName: String, val schoolBorough:String, val schoolSummary: String, val onClick: () -> Unit) {
   fun getUniqueKey() = schoolName.hashCode()
}